...
from Sketchformer.utils.helpers import Timer, save_pickle, load_pickle
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class-list', type=str, default='./small_quickdraw/list_quickdraw.txt')
    ...
    args = parser.parse_args()
    print("Loading data ...")
    #selected_classes = np.random.choice(class_files,35)
    #data_p0, data_p1 = load_data(selected_classes)
    ...
    diction = load_pickle(args.target_file)
    

    """
    print( "cluster centers " , diction.cluster_centers_)
    try:
        print("data shape", data.shape)
        label = diction.predict(data[:100000,:])
    except e:
        raise e
    ...
    #u_labels = np.random.choice(np.unique(label), 10)
    u_labels = np.unique(label)
    print("u_labels ", u_labels)
    #plotting the results:
    for i in range(100):
        #plt.scatter(data[:100000,:][label == i , 0] , data[:100000,:][label == i , 1] , label = i)
        plt.scatter([diction.cluster_centers_[i, 0]], [diction.cluster_centers_[i, 1]] , color = 'black', marker="x")
    plt.legend()
    plt.xlabel("dx")
    plt.ylabel("dy")
