...
from Sketchformer.utils.sketch import convert_to_absolute, to_relative
"""
...
def strokes_to_lines_alternative(strokes, scale=1.0, start_from_origin=False):
    """
...
def apply_RDP(strokes):
    ...
    try:
        ...
        los = strokes_to_lines_alternative(strokes)
        simplified_sketch = []

        for stroke in los:
            abs_stroke = convert_to_absolute(stroke)
            print("abs_stroke ", abs_stroke)
            simplified_stroke = rdp(abs_stroke, epsilon=2.0)
            print("simplified_stroke ", simplified_stroke)
            rel_stroke = to_relative(simplified_stroke)
            arr_stroke = np.asarray(rel_stroke)
        ...
        print("simplified_sketch ", simplified_sketch)
...
def read_svg(svg_path, scale=100.0, draw_mode=False):
    ...
    try:
        ...
        # normalised
        print("lines before grouped ", lines)
        lines = [group(path) for path in lines]
        print("lines after grouped ", lines)
        lines_simplified = [rdp(path, epsilon=2.0) for path in lines]  # apply RDP algorithm epsilon=1.5 
        '''
        lines_simplified = []
        for path in lines:
            print("path ", path)
            temp = rdp(path, epsilon=2.0)
            print("temp ", temp)
            lines_simplified.append(temp)
        '''
        print("lines after simplified ", lines_simplified)
        strokes_simplified = lines_to_strokes(lines_simplified)  # convert to 3-stroke format (dx,dy,pen_state)
        #print("strokes_simplified ", strokes_simplified)
